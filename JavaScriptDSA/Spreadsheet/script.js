...
const median = (nums) => {
  const sorted = nums.slice().sort((a, b) => a - b)   // Ascending sort
...
const spreadsheetFunctions = {
  ...
  random: (nums) => {
    const num1 = nums[0]
    const num2 = nums[1]
    const num_min = Math.min(num1, num2)
    const num_range = Math.max(num1, num2) - num_min
    return Math.floor(Math.random() * (num_range) + num_min)
  },
  */
...
const applyFunction = (str) => {
  ...
  // const infix = /([0-9]+(.[0-9]+)?)([+-])([0-9]+(.[0-9]+)?)/
  const infix = /([\d.]+)([+-])([\d.]+)/
  const str2 = infixEval(noHigh, infix)
  // Look for things like "sum(1, 4)"
...
// Load the sheet in the beginning
window.onload = () => {
